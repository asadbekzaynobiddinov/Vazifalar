...
export const isAdmin = async (req, res, next) => {
    try {
        ...
        if(user.role != 'Admin'){
            return res.status(403).json({message: 'Sizda Admin huquqlari mavjud emas'})
        }

        req.user = user
        next()
    } catch (error) {
        return res.status(401).send({message: 'Token yaroqsiz yoki eskirgan', error: error.message})        
    }
}
...
export const checkRole = (roles) => async (req, res, next) => {
    try {
        ...
        if(!roles.includes(user.role)){
            return res.status(403).json({message: `Sizda ${roles.join(' yoki ')} huquqlari mavjud emas`})
        }
